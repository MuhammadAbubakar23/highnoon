...
@Component({
  selector: 'app-encashment-team',
  templateUrl: './encashment-team.component.html',
  styleUrls: ['./encashment-team.component.css']
})
export class EncashmentTeamComponent {
  pageNumber = 1;
  pageSize = 10;
  totalPages = 0;
  totalRecords = 0;
  ...
  constructor(private _hS: HeaderService, private _lUS: LeaveUtilizationService, private _toastS: ToasterService, private _dropdownS: DropDownApiService
    , private datePipe: DatePipe, private spinner: NgxSpinnerService, private _per: MenuPermissionService, private _route: Router) {
      ...
      if (this._per.hasPermission('My Leaves')) {
        tabs.push({ title: 'Leave Encashment', url: 'connect/leaves/encashment', isActive: false })
      }
      if (this._per.hasPermission('Team Leaves')) {
        tabs.push( { title: 'Team Requests', url: 'connect/leaves/encashment/team-requests', isActive: true })
      }
      else {
        isTabActive = false;
        this._route.navigateByUrl('/connect/encashment');
      }
  ...
  getTeamRequests() {
    ...
    this._lUS.getTeamLeaveUtilizations(data).subscribe((res) => {
      ...
      this.encashmentItems.forEach(element => {
  ...
  updateRequest(reqstageId: number, statusId: number,content) {
    ...
    this._lUS.updateRequestStage(data).subscribe((res) => {
      if (res.statusCode === 200) {
        let message = ""
        if (statusId === 2) {
          message = "You have approved the Encashment successfully."
        }
        if (statusId === 3) {
          message = "You have rejected the  Encashment."
        }
  ...
  getLeaveSummary(requester,content) {
    ...
    this._lUS.getLeaveUtilRequestSummary(requester.leaveUtilizationId).subscribe(res => {
      this.requesterDetails = {
        ...
        "appliedDate": this.datePipe.transform(res.date.appliedDate, 'MMM dd, yyyy'),
      }
